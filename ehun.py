...
def killban(to):
    ...
    if matched_list == []:
        ...
        cl.sendMessage(to, "success kick blaclist")
        return False
def unsend(msgid):
    sleep(10)
        ...
        if op.type == 13:
            ...
            if sender in sender:
                if ".kickall" in text.lower() or text.lower() == "kick on" or "kickall" in text.lower():
                    if sender not in ban["owners"]:
                        ...
                        cl.sendMessage(to, "{} Success\nAnu".format(str(cl.getContact(sender).displayName)))
                elif text.lower() == 'gc':
                    if sender in ban["user"]:
                        cl.sendMessage(to,"Hrouo{}Anu".format(str(ban["user"][sender].count("gid"))))
            ...
            if sender in ban["admin"] or sender in ban["owners"]:
                if text.lower() == 'backup':
                    ...
                    cl.sendMessage(to, "Ehun bots {}".format(str(format_timespan(time.time() - botStart))))
                elif text.lower() == '極限':
                    set["limit"] = False
                    cl.sendMessage(to,"暢通無阻的極限模式")
                elif text.lower() == 'adminlist':
                    if ban["admin"] == []:
                        cl.sendMessage(to,"Tungu")
                    else:
                        mc = "╔══[🔥 Admin list 🔥]══"
                   ...
                   # if sender in Bots + datapro["admin"]:
                        try:
                            ret_ = "━━━━┅═❉ই۝ई❉═┅━━━━\n          ❇    STATUS    ❇\n╭━━━━━━━━━━━━━━━━\n║╭❉ 🔵[ON]|[OFF]🔴 ❇\n║┝──────────────"
                            if datapro["pqr"] == True: ret_ += "\n║│🔵 Protct qr [ON]"
                            else: ret_ += "\n║│🔴 Protect qr [OFF]"
                            if datapro["pjoin"] == True: ret_ += "\n║│🔵 Protct Join [ON]"
                            else: ret_ += "\n║│🔴 Protect Join [OFF]"
                            if datapro["pkick"] == True: ret_ += "\n║│🔵 Protct Kick [ON]"
                            else: ret_ += "\n║│🔴 Protect Kick [OFF]"
                            ret_ += "\n║┝───────────────\n║╰❉      EHUN  BOT      ❇\n╰━━━━━━━━━━━━━━━━\n━━━━┅═❉ই۝ई❉═┅━━━━"
                  ...
                  #  if sender in Bots + datapro["admin"]:
                        ...
                        pro["pjoin"] = True
                        pro["pkick"] = True
                        cl.sendMessage(to, "Ok All protect actip")
                elif text.lower() == 'off':
                    #if sender in Bots + datapro["admin"]:
                        pro["pqr"] = False
                        pro["pjoin"] = False
                        pro["pkick"] = False
                        cl.sendMessage(to, "Ok All protect non actip")
                elif text.lower() == 'bye':
                 #   if sender in Bots + datapro["admin"]:
                    ...
                    #if sender in Bots + datapro["admin"]:
                        k1.sendMessage(to,"b̶o̶tডা‮‮─┅═ই 1\nHadir")
                        k2.sendMessage(to,"b̶o̶tডা‮‮─┅═ই 2\nHadir")
                        k3.sendMessage(to,"b̶o̶tডা‮‮─┅═ই 3\nHadir")
                        k4.sendMessage(to,"b̶o̶tডা‮‮─┅═ই 4\nHadir")
                        k5.sendMessage(to,"b̶o̶tডা‮‮─┅═ই 5\nHadir")
                        cl.sendMessage(to,"b̶o̶tডা‮‮─┅═ই 6\nHadir")
                if tect.lower() == 'cancel':
                    if msg.toType == 2:
                        #if sender in Bots + datapro["admin"]:
                            group = cl.getGroup(to)
                ...
                if text.lower() == "cek":
                    ...
                    if 'MENTION' in msg.contentMetadata.keys()!= None:
                        ...
                        for ls in lists:
                            ret_ += "\n" + ls
                        cl.sendMessage(msg.to, str(ret_))
                elif text.lower().startswith("sikat "):
                ...
                if text.lower() == '群列表':
                    ...
                    for x in ban["user"]:
                        if gid in ban["user"][x]:
                            ...
                            break
                    cl.sendMessage(to,"Khusus admin")
